...
$uid = $_SESSION['uid'];
$currency=$GLOBALS ['mysqli']->real_escape_string($_POST['currency']);
$category=$GLOBALS ['mysqli']->real_escape_string($_POST['category']);
$membership=$GLOBALS ['mysqli']->real_escape_string($_POST['membership']);

if($currency=='')
{
    print "err||Please select currency.";
    exit;
}
if($category=='')
{
    print "err||Please select category.";
    exit;
}
if($membership=='')
{
    print "err||Please select membership type.";
    exit;
}
...
//print $category.'  '.$membership.'  '.$amount;
//exit;
...
if($currency=='YEM')
{
    ...
    if($tbalance<$amount)
    {
        ...
        print "err||You do not have sufficient YEM balance.";
        exit;
    }
    ...
    if($result[0]=='success')
    {
        ...
        $select = "insert into ivc_membership (uid, category, membership, currency, amount, date_added, date_payment, hash, paid_status) values ($uid, '$category', '$membership', '$currency', $amount, NOW(), NOW(), '$hash', 'paid')";
        //print $select;
...
else
{
    ...
    $select = "insert into ivc_membership (uid, category, membership, currency, amount, date_added) values ($uid, '$category', '$membership', '$currency', $amount, NOW())";
    //print $select;
    $GLOBALS ['mysqli']->query ($select) or die ($GLOBALS ['mysqli']->error . __LINE__);
